// ==UserScript==
// @name         EX-百度云盘，网盘直链直接下载助手增强版
// @namespace    wuxingsanren
// @version      2.2.4
// @description  EX-百度云盘，网盘直链直接下载助手增强版，[下载大文件] [批量下载] [文件夹下载] [百度网盘] [百度云盘] [百度云盘企业版] [企业版] [baidu] [baiduyun] [yunpan] [baiduyunpan] [eyun]
// @author       wuxingsanren
// @iconURL      https://pan.baidu.com/box-static/disk-header/header/img/user-level2.png
// @resource     vipLevel https://pan.baidu.com/box-static/disk-header/header/img/user-level2.png
// @resource     wxappQrcode https://cdn.dgsldz.com/product/wildcat/images/qrcode/wemp_wuxingsanren.jpg
...
// @match        *://pan.baidu.com/share/link*
// @match        *://yun.baidu.com/share/link*
// @match        *://pandownload.com/*
// @match        *://www.baiduwp.com/*
// @match        *://xdown.org/*
...
(function () {
  ...
  function addDiv() {
    ...
    var wxappQrcodeImg = GM_getResourceURL("wxappQrcode");
    div.innerHTML =
      "<img src='" +
      vipLevelImg +
      "' style='width:20px;height:20px;position:relative;top:6px;' /></a>&nbsp;百度网盘SVIP<br /><span style='position:relative;top:5px;color:red;'>使用前先看说明</span><br /><span style='color:#3b8cff;'>http://yemao.in/wangpan</span><br /><iframe src='https://ghbtns.com/github-btn.html?user=wuxingsanren&repo=wildcat-vip-account&type=star&count=true' frameborder='0' style='height:20px;'></iframe>";
    var css =
      "width:115px;height:32px;line-height:32px;position:fixed;bottom:255px;left:20px;z-index:999999999;text-align:center;font-size:10px;background:#fff;font-family:Verdana, Arial, '宋体';font-weight:500;color:#d07d12;background-color:#ffd821;user-select:none;padding:0px;white-space:nowrap;border-radius:6px;border:2px solid #ffd821;cursor:pointer;";
    ...
    div.addEventListener("click", function () {
      ...
      url = "https://github.com/wuxingsanren/wildcat-vip-account";
  ...
  function PanHelper() {
    ...
    function initParams() {
      ...
      if (currentPage == "list") currentPath = getPath();

      if (currentPage == "category") currentCategory = getCategory();

      if (currentPage == "search") searchKey = getSearchKey();
    ...
    function getListGridStatus() {
      ...
      return $("div." + wordMap["list-grid-switch"]).hasClass(
        wordMap["list-switched-on"]
      )
        ? "list"
        : $("div." + wordMap["list-grid-switch"]).hasClass(
            wordMap["grid-switched-on"]
          )
        ? "grid"
        : "list";
    }
      ...
      $dd.each(function (index, element) {
        $(element).bind("click", function (e) {
          ...
          if (nodeName != "span" && nodeName != "a" && nodeName != "em") {
            ...
            if (!e.shiftKey) {
              selectFileList = [];
              var filename = $("div.file-name div.text a", $(this)).attr(
                "title"
              );
              slog("选中文件：" + filename);
              $.each(fileList, function (index, element) {
              ...
              var $dd_select = $(
                "div." + wordMap["list-view"] + " dd." + wordMap["item-active"]
              );
              $.each($dd_select, function (index, element) {
                var filename = $("div.file-name div.text a", $(element)).attr(
                  "title"
                );
                slog("选中文件：" + filename);
                $.each(fileList, function (index, element) {
    ...
    function addButton() {
      ...
      var $dropdownbutton_a = $(
        '<a class="g-button" href="javascript:void(0);"><span class="g-button-right"><em class="icon icon-download" title="百度网盘下载助手"></em><span class="text" style="width: auto;">下载助手</span></span></a>'
      );
      var $dropdownbutton_span = $(
        '<span class="menu" style="width:96px"></span>'
      );

      var $directbutton = $(
        '<span class="g-button-menu" style="display:block"></span>'
      );
      var $directbutton_span = $(
        '<span class="g-dropdown-button g-dropdown-button-second" menulevel="2"></span>'
      );
      var $directbutton_a = $(
        '<a class="g-button" href="javascript:void(0);"><span class="g-button-right"><span class="text" style="width:auto">直接下载</span></span></a>'
      );
      var $directbutton_menu = $(
        '<span class="menu" style="width:120px;left:79px"></span>'
      );
      var $directbutton_download_button = $(
        '<a id="download-direct" class="g-button-menu" href="javascript:void(0);">下载</a>'
      );
      var $directbutton_link_button = $(
        '<a id="link-direct" class="g-button-menu" href="javascript:void(0);">显示链接</a>'
      );
      var $directbutton_batchhttplink_button = $(
        '<a id="batchhttplink-direct" class="g-button-menu" href="javascript:void(0);">批量链接(HTTP)</a>'
      );
      var $directbutton_batchhttpslink_button = $(
        '<a id="batchhttpslink-direct" class="g-button-menu" href="javascript:void(0);">批量链接(HTTPS)</a>'
      );
      $directbutton_menu
        .append($directbutton_download_button)
        .append($directbutton_link_button)
        .append($directbutton_batchhttplink_button)
        .append($directbutton_batchhttpslink_button);
      $directbutton.append(
        $directbutton_span.append($directbutton_a).append($directbutton_menu)
      );
      ...
      var $apibutton_a = $(
        '<a class="g-button" href="javascript:void(0);"><span class="g-button-right"><span class="text" style="width:auto">API下载</span></span></a>'
      );
      var $apibutton_menu = $(
        '<span class="menu" style="width:120px;left:77px"></span>'
      );
      var $apibutton_download_button = $(
        '<a id="download-api" class="g-button-menu" href="javascript:void(0);">下载</a>'
      );
      var $apibutton_link_button = $(
        '<a id="httplink-api" class="g-button-menu" href="javascript:void(0);">显示链接</a>'
      );
      var $apibutton_batchhttplink_button = $(
        '<a id="batchhttplink-api" class="g-button-menu" href="javascript:void(0);">批量链接(HTTP)</a>'
      );
      var $apibutton_batchhttpslink_button = $(
        '<a id="batchhttpslink-api" class="g-button-menu" href="javascript:void(0);">批量链接(HTTPS)</a>'
      );
      $apibutton_menu
      ...
      $apibutton.append(
        $apibutton_span.append($apibutton_a).append($apibutton_menu)
      );
      ...
      var $outerlinkbutton_a = $(
        '<a class="g-button" href="javascript:void(0);"><span class="g-button-right"><span class="text" style="width:auto">外链下载</span></span></a>'
      );
      var $outerlinkbutton_menu = $(
        '<span class="menu" style="width:120px;left:79px"></span>'
      );
      var $outerlinkbutton_download_button = $(
        '<a id="download-outerlink" class="g-button-menu" href="javascript:void(0);">下载</a>'
      );
      var $outerlinkbutton_link_button = $(
        '<a id="link-outerlink" class="g-button-menu" href="javascript:void(0);">显示链接</a>'
      );
      var $outerlinkbutton_batchlink_button = $(
        '<a id="batchlink-outerlink" class="g-button-menu" href="javascript:void(0);">批量链接</a>'
      );
      $outerlinkbutton_menu
        .append($outerlinkbutton_download_button)
        .append($outerlinkbutton_link_button)
        .append($outerlinkbutton_batchlink_button);
      $outerlinkbutton.append(
        $outerlinkbutton_span
          .append($outerlinkbutton_a)
          .append($outerlinkbutton_menu)
      );
    ...
    function downloadClick(event) {
      ...
      if (id == "download-direct") {
        ...
        if (selectFileList.length === 0) {
          ...
          if (selectFileList[0].isdir === 1) downloadType = "batch";
          else if (selectFileList[0].isdir === 0) downloadType = "dlink";
        ...
        if (result.errno === 0) {
          if (downloadType == "dlink") downloadLink = result.dlink[0].dlink;
            ...
            if (selectFileList.length === 1)
              downloadLink =
                downloadLink +
                "&zipname=" +
                encodeURIComponent(selectFileList[0].filename) +
                ".zip";
          } else {
            alert("发生错误！");
            return;
        ...
        if (id == "download-api") {
          downloadLink = getDownloadLinkWithRESTAPIBaidu(
            selectFileList[0].path
          );
    ...
    function linkClick(event) {
      ...
      if (id.indexOf("direct") != -1) {
        ...
        $.each(selectFileList, function (index, element) {
          if (selectFileList.length == 1) filename = element.filename;
          else {
            if (index == 0) filename = element.filename;
            else filename = filename + "," + element.filename;
          }
        });
        linkList = {
          filename: filename,
        ...
        tip =
          "显示模拟百度网盘网页获取的链接，可以使用右键迅雷下载，复制到下载工具需要传递cookie，多文件打包下载的链接可以直接复制使用";
        dialog.open({
          title: "下载链接",
          type: "link",
          list: linkList,
        ...
        if (id.indexOf("api") != -1) {
          var downloadLink = getDownloadLinkWithRESTAPIBaidu(
            selectFileList[0].path
          );
          var httplink = downloadLink.replace(/^([A-Za-z]+):/, "http:");
          var httpslink = downloadLink.replace(/^([A-Za-z]+):/, "https:");
          ...
          tip =
            "显示模拟APP获取的链接(使用百度云ID)，可以使用右键迅雷下载，复制到下载工具需要传递cookie";
          dialog.open({
            title: "下载链接",
            type: "link",
            list: linkList,
          ...
          tip =
            "显示模拟百度网盘客户端获取的链接，可以直接复制到下载工具使用，不需要cookie";
          dialog.open({
            title: "下载链接",
            type: "link",
            list: linkList,
    ...
    function getDirectBatchLink(linkType) {
      ...
      $.each(selectFileList, function (index, element) {
        ...
        if (element.isdir == 0) downloadType = "dlink";
        else downloadType = "batch";
        fid_list = getFidList([element]);
        result = getDownloadLinkWithPanAPI(downloadType);
        if (result.errno == 0) {
          if (downloadType == "dlink") downloadLink = result.dlink[0].dlink;
          else if (downloadType == "batch") downloadLink = result.dlink;
    ...
    function getDownloadLinkWithRESTAPIBaidu(path) {
      var link =
        restAPIUrl +
        "file?method=download&app_id=250528&path=" +
    ...
    function getDownloadLinkWithRESTAPIES(path) {
      var link =
        restAPIUrl +
        "file?method=download&app_id=266719&path=" +
        encodeURIComponent(path);
      return link;
    }
    ...
    function getDownloadLinkWithClientAPI(path) {
      ...
      var url =
        clientAPIUrl +
        "file?method=locatedownload&app_id=250528&ver=4.0&path=" +
        encodeURIComponent(path);
      $.ajax({
        url: url,
        method: "POST",
  ...
  function PanShareHelper() {
    ...
    function addButton() {
      ...
      var $dropdownbutton = $('<span class="g-dropdown-button"></span>');
      var $dropdownbutton_a = $(
        '<a class="g-button" data-button-id="b200" data-button-index="200" href="javascript:void(0);"></a>'
      );
      var $dropdownbutton_a_span = $(
        '<span class="g-button-right"><em class="icon icon-download" title="百度网盘下载助手"></em><span class="text" style="width: auto;">下载助手</span></span>'
      );
      var $dropdownbutton_span = $(
      ...
      var $downloadButton = $(
        '<a data-menu-id="b-menu207" class="g-button-menu" href="javascript:void(0);">直接下载</a>'
      );
      var $linkButton = $(
        '<a data-menu-id="b-menu208" class="g-button-menu" href="javascript:void(0);">显示链接</a>'
      );
      ...
      $("div.module-share-top-bar div.bar div.button-box").append(
        $dropdownbutton
      );
    }
    ...
    function confirmClick() {
      ...
      if (result.errno == -20) {
        ...
        if (buttonTarget == "download") {
          ...
          var linkList = {
            filename: filename,
            urls: [{ url: link, rank: 1 }],
          };
          var tip =
            "显示获取的链接，可以使用右键迅雷下载，复制无用，需要传递cookie";
          dialog.open({
            title: "下载链接",
            type: "link",
            list: linkList,
    ...
    function linkButtonClick() {
      ...
      if (downloadLink.errno == -20) {
        ...
        else $("#dialog-downloadlink").attr("href", link).text(link);
        var filename = "";
        $.each(selectFileList, function (index, element) {
          if (selectFileList.length == 1) filename = element.filename;
          else {
  ...
  function Dialog() {
    ...
    function createDialog() {
      ...
      var $dialog_div = $(
        '<div class="dialog" style="width: 800px; top: 0px; bottom: auto; left: ' +
          dialogLeft +
      ...
      $dialog_div
        .append($dialog_header.append($dialog_control))
        .append($dialog_body);
      ...
      $dialog_radio_div
        .append($dialog_radio_multi)
        .append($dialog_radio_single);
      $dialog_div.append($dialog_radio_div);
      $("input[type=radio][name=showmode]", $dialog_radio_div).change(
        function () {
          var value = this.value;
          var $textarea = $(
            "div.dialog-body textarea[name=dialog-textarea]",
            dialog
          );
      ...
      $dialog_button_div
        .append($dialog_copy_button)
        .append($dialog_edit_button)
        .append($dialog_exit_button);
      ...
      $dialog_copy_button.click(function () {
        ...
        if (showParams.type == "batch") {
          $.each(linkList, function (index, element) {
            if (element.downloadlink == "error") return;
            if (index == linkList.length - 1)
              content = content + element.downloadlink;
            else content = content + element.downloadlink + "\n";
          });
        } else if (showParams.type == "link") {
          $.each(linkList, function (index, element) {
            if (element.url == "error") return;
            if (index == linkList.length - 1) content = content + element.url;
            else content = content + element.url + "\n";
          });
        }
    ...
    this.open = function (params) {
      ...
      if (params.type == "link") {
        ...
        $("div.dialog-header h3 span.dialog-title", dialog).text(
          params.title + "：" + params.list.filename
        );
        $.each(params.list.urls, function (index, element) {
          var $div = $(
            '<div><div style="width:30px;float:left">' +
              element.rank +
        ...
        if (params.showall) {
          $.each(params.list, function (index, element) {
            ...
            var $item_name = $(
              '<div style="width:100px;float:left;overflow:hidden;text-overflow:ellipsis" title="' +
                element.filename +
                '">' +
                element.filename +
                "</div>"
            ...
            var $item_first = $(
              '<div class="item-first" style="overflow:hidden;text-overflow:ellipsis"><a href="' +
            ...
            $.each(params.alllist[index].links, function (n, item) {
              if (element.downloadlink == item.url) return;
              var $item = $(
                '<div class="item-ex" style="display:none;overflow:hidden;text-overflow:ellipsis"><a href="' +
                  item.url +
                  '">' +
                  item.url +
                  "</a></div>"
          ...
          $.each(params.list, function (index, element) {
            var $div = $(
              '<div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><div style="width:100px;float:left;overflow:hidden;text-overflow:ellipsis" title="' +
                element.filename +
                '">' +
                element.filename +
  ...
  function VCodeDialog(refreshVCode, confirmClick) {
    ...
    function createDialog() {
      ...
      var $dialog_div = $(
        '<div class="dialog" id="dialog-vcode" style="width:520px;top:0px;bottom:auto;left:' +
          dialogLeft +
      ...
      var $dialog_verify_body = $(
        '<div class="verify-body">请输入验证码：</div>'
      );
      var $dialog_input = $(
      ...
      $dialog_verify_body
        .append($dialog_input)
        .append($dialog_img)
        .append($dialog_refresh);
      $dialog_body_download_verify
        .append($dialog_verify_body)
        .append($dialog_err);
      $dialog_body_div.append($dialog_body_download_verify);
      $dialog_body.append($dialog_body_div);
      $dialog_footer
        .append($dialog_confirm_button)
        .append($dialog_cancel_button);
      $dialog_div
        .append($dialog_header)
        .append($dialog_body)
        .append($dialog_footer);
      $("body").append($dialog_div);
